(function($, window) {
	...
	var template = '<div id="{{id}}" class="mui-slider mui-preview-image mui-fullscreen"><div class="mui-preview-header">{{header}}</div><div class="mui-slider-group"></div><div class="mui-preview-footer mui-hidden">{{footer}}</div><div class="mui-preview-loading"><span class="mui-spinner mui-myselfloading"></span></div></div>';
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			zoom: true,
			header: '',
			footer: '<div class="mui-preview-indicator"></div>'
		}, options || {});
		this.init();
		this.initEvent();
	};
	...
	proto.initEvent = function() {
		...
		this.element.addEventListener('slide', function(e) {
			...
			// 修改数字成对应点
			var TagSpan = self.indicator.getElementsByTagName('span');
			for(var i=0; i<TagSpan.length; i++) {
				TagSpan[i].className ='';
			}
			TagSpan[slideNumber].className = 'active';
		});
	};
	// 页面载入图片开始
	proto.addImages = function(group, index) {
		...
		if (imgs.length) {
			for (var i = 0, len = imgs.length; i < len; i++) {
				...
				this.indicator.appendChild(_span);
			}
		}
	};
	// 遍历载入的图片
	...
	proto._loadItem = function(index, isOpening) { //TODO 暂时仅支持img
		// 修改数字成对应点
		var TagSpan = this.indicator.getElementsByTagName('span');
		TagSpan[index].className = 'active';
